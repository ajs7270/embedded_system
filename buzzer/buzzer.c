...
#define LOG(x, ...) printk(KERN_INFO "fpga-gpio"": "x"\n", ##__VA_ARGS__)
...
ssize_t iom_buzzer_write(struct file *inode, const char *gdata, size_t length, loff_t *off_what) 
{
    ...
    _s_value = value & 0xF;
        ...
        iom_fpga_itf_write((unsigned int)IOM_BUZZER_ADDRESS, _s_value);
...
ssize_t iom_buzzer_read(struct file *inode, char *gdata, size_t length, loff_t *off_what) 
{
	...
	_s_value = iom_fpga_itf_read((unsigned int)IOM_BUZZER_ADDRESS);
